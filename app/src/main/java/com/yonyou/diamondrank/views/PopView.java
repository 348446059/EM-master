...
import com.yonyou.diamondrank.utils.ContextUtils;
...
//                vote
                if (voteContent.getText().toString().trim().equals("")){
                    ...
                    if (voteContent.getText().toString().trim().length() < 5 || countLevel() == 0){
                        ToastUtils.makeShortText("理由不得少于五个字,并且钻石数不能小于1",context);
                    }else {
                        OkHttpUtils
                                .post()//
                                .url(Utils.getUrl(context,AppConstants.VOTE_URL))
